...
def check_Ajia(start_time,end_time,zuoye):
    # if start_time == '2024-05-18 09:33:50': #2024-05-23 23:05:49   2024-05-31 21:12
    #     print('debug')
    segment = df[(df['csvTime'] >= start_time) & (df['csvTime'] <= end_time) & (df['check_current_presence'].isin(['有电流', '无电流']))]
    if segment.empty:
    ...
    dp_overlap = any(dp_end >= start_dt and dp_start <= end_dt for dp_start, dp_end in dp_segments)
    device_overlap = any(dev_end >= start_dt and dev_start <= end_dt for dev_start, dev_end in df_device_segments)
...
if __name__ == "__main__":
    ...
    for idx, row in df_DP.iterrows():
        if row['status'] == 'ON DP':
            dp_start = pd.to_datetime(row['csvTime'])
        elif row['status'] == 'OFF DP' and dp_start:
            dp_end = pd.to_datetime(row['csvTime'])
            dp_segments.append((dp_start, dp_end))
            dp_start = None
    ...
    for i in range(1, df_device.shape[0]):
        ...
        if df_device.iloc[i - 1]['13-11-6_v_new'] < 10 and df_device.iloc[i]['13-11-6_v_new'] >= 10 \
            and df_device.iloc[i]['13-11-6_v_new'] - df_device.iloc[i-1]['13-11-6_v_new'] >= 3   and df_device.iloc[i]['status'] != '折臂吊车开机': 
        ...
        if df_device.iloc[i - 1]['13-11-6_v_new'] >= 10 and df_device.iloc[i]['13-11-6_v_new'] < 10 \
            and df_device.iloc[i - 1]['13-11-6_v_new'] - df_device.iloc[i]['13-11-6_v_new'] >= 3 and df_device.iloc[i-1]['status'] != '折臂吊车开机':
    ...
    # Convert segments tuples to datetime and create a copy
    df_device_segments = [(pd.to_datetime(start), pd.to_datetime(end)) for start, end in segments]

    # 提取每个区段内的“由待机进入工作”和“由工作进入待机”事件,并更新status（小艇检查完毕、入水、落座）
    ...
    def extract_events(df, segment,zuoye):
        start, end = segment
    ...
    def device(csvTime):
        ...
        if matching_rows.empty:
            print(f'device找不到匹配的时间戳{csvTime}')
            return None
        ...
        for i in range(row_idx-1, start_row, -1):
            # Check for '小艇入水'
            if cnt == 0 and df_device.loc[i, '13-11-6_v'] < 10 and df_device.loc[i-1, '13-11-6_v'] >= 10 \
                and (df_device.loc[i-1, '13-11-6_v'] - df_device.loc[i, '13-11-6_v']) >= 3 and df_device.loc[i-1, 'status'] == 'False':
            ...
            # Check for '小艇检查完毕'
            if cnt == 1 and i > start_row + 1 and df_device.loc[i, '13-11-6_v'] < 10 and df_device.loc[i-1, '13-11-6_v'] >= 10 \
                and (df_device.loc[i-1, '13-11-6_v'] - df_device.loc[i, '13-11-6_v']) >= 3 and df_device.loc[i-1, 'status'] == 'False':
        ...
        if end_row is not None:
            for i in range(end_row, row_idx, -1):
                ...
                if df_device.loc[i, '13-11-6_v'] < 10 and df_device.loc[i-1, '13-11-6_v'] >= 10 and (df_device.loc[i-1, '13-11-6_v'] - df_device.loc[i, '13-11-6_v']) >= 3:
                    df_device.loc[i-1, 'status'] = '小艇落座'
                    break
        
        if cnt < 2 :
            print('找不到小艇入水或小艇检查完毕:',df_device.loc[start_row, 'csvTime'],df_device.loc[end_row, 'csvTime'])
            return None
        return boat_water_time  # Return the time when boat entered water

    def xiafang(segment):
        if segment[0] == '2024-10-18 07:54:50': #2024-06-23 08:05:43
            print('debug')
        segment_start = pd.to_datetime(segment[0])
        segment_end = pd.to_datetime(segment[1])
        
        crane_segments = []
        # Step 1: 找出A架开关机区间内，与segment有重叠的折臂吊车开关机时间段
        for current_start, current_end in df_device_segments:
            # 若该折臂吊车区间与A架segment有重叠，则记录
            if current_end >= segment_start and current_start <= segment_end:
                crane_segments.append((current_start, current_end))

        success = False

        # Step 2: 遍历每一个折臂吊车开关机时间段，直到动作判定成功
        for crane_start, crane_end in crane_segments:
            # 2.1 判断该折臂吊车区间是否有重叠的DP开启时段
            has_overlap = False
            for dp_start, dp_end in dp_segments:
                if dp_end >= crane_start and dp_start <= crane_end:
                    has_overlap = True
                    break
                    
            if not has_overlap:
                print('折臂吊车区间内没有与DP开启时段重叠')
                continue

            # 2.2 统计该区间内的‘由待机进入工作’与‘由工作进入待机’事件数
            ...
            if work_events.shape[0] < 6: # 该区间内的‘由待机进入工作’与‘由工作进入待机’事件数小于6 （条件有待商榷，是否存在回收/下放阶段，折臂吊车动作缺失？或者是工作状态判定有问题？ 例如5.27上午A架下放阶段，折臂吊车里没有找到全部的动作）
                print('折臂吊车区间内的‘由待机进入工作’与‘由工作进入待机’事件数小于6')
                continue

            # Step 3: 确定折臂吊车区间与A架区间的重叠部分
            overlap_start = max(segment_start, crane_start)
            overlap_end = min(segment_end, crane_end)
            if overlap_start >= overlap_end:
                continue


            # 3.1 将重叠区间内的通电流区间提取并传入extract_events
            overlap_start_str = overlap_start.strftime("%Y-%m-%d %H:%M:%S")
            overlap_end_str = overlap_end.strftime("%Y-%m-%d %H:%M:%S")
            overlap_events = df[
                (df['csvTime'] >= overlap_start_str) & (df['csvTime'] <= overlap_end_str)
            ]
            # 保存重叠区间首尾行原始的check_current_presence状态
            first_index = overlap_events.index[0]
            last_index = overlap_events.index[-1]
            orig_first_status = df.loc[first_index, 'check_current_presence']
            orig_last_status = df.loc[last_index, 'check_current_presence']
            first = False
            last = False
            # 若首行Ajia-3_v和Ajia-5_v均大于0且check_current_presence为False，则标记为'有电流'
            if df.loc[first_index, 'Ajia-3_v_new'] > 0 and df.loc[first_index, 'Ajia-5_v_new'] > 0 and df.loc[first_index, 'check_current_presence'] == 'False':
                df.loc[first_index, 'check_current_presence'] = '有电流'
                first = True

            # 若尾行Ajia-3_v和Ajia-5_v均大于0且check_current_presence为False，则标记为'无电流'
            if df.loc[last_index, 'Ajia-3_v_new'] > 0 and df.loc[last_index, 'Ajia-5_v_new'] > 0 and df.loc[last_index, 'check_current_presence'] == 'False':
                df.loc[last_index, 'check_current_presence'] = '无电流'
                last = True

            # 调用extract_events函数
            events_list, peak_list = extract_events(df, (overlap_start_str, overlap_end_str), '布放')

            # 恢复首尾行原始的check_current_presence状态
            if first:
                df.loc[first_index, 'check_current_presence'] = orig_first_status
            if last:
                df.loc[last_index, 'check_current_presence'] = orig_last_status

            if sum(events_list) < 2:
                print('峰值数量小于2', (overlap_start_str, overlap_end_str))
                continue

            # 3.2 提取重叠区间内的A架数据，判定峰值、缆绳解除及出水等动作
            data1 = overlap_events[['Ajia-3_v_new', 'Ajia-5_v_new']].values.tolist()

            # 在多个峰值中，是先确定征服者起吊、入水，再找之后的最大峰值标记为A架摆回。如果先确定征服者入水，那他应该是第一个峰值所处段？
            # 还是先找最大峰值标记为A架摆回，再找之前的征服者起吊、入水？
            # 结合折臂吊车的工作段，缆绳解除对应的时间点前应该至少有两段工作区间。（先确定征服者起吊、入水，再找之后的最大峰值标记为A架摆回）
            
            
            # indices = [i for i, x in enumerate(data1) if x[1] == peak_list[0]]
            chosen_peak = None
            for candidate in peak_list:
                value = find_first_stable_after_peak(data1, candidate)
                candidate_rows = overlap_events[overlap_events['Ajia-5_v_new'] == value]

                if candidate_rows.empty:
                    continue
                candidate_time = candidate_rows.iloc[0]['csvTime']
                work_events = df_device[
                    (df_device['csvTime'] >= crane_start.strftime("%Y-%m-%d %H:%M:%S")) &
                    (df_device['csvTime'] <= candidate_time) &
                    (df_device['action'].isin(['由待机进入工作', '由工作进入待机']))
                ]
                if work_events.shape[0] >= 4:
                    chosen_peak = candidate
                    break
            if chosen_peak is not None:
                indices = [i for i, x in enumerate(data1) if x[1] == chosen_peak]
                if indices:
                    idx = indices[0]
                    # 向前和向后扩展以获取完整的通电流段（两边的数值均大于0）
                    start_index = idx
                    while start_index > 0 and data1[start_index - 1][0] > 0 and data1[start_index - 1][1] > 0:
                        start_index -= 1
                    end_index = idx
                    while end_index < len(data1) - 1 and data1[end_index + 1][0] > 0 and data1[end_index + 1][1] > 0:
                        end_index += 1
                    segment_data = data1[start_index:end_index + 1]
                    value_11 = find_first_increasing_value(segment_data)
                else:
                    value_11 = None
            else:
                print('没有找到符合条件的峰值，缆绳解除对应的时间点前应该至少有两段工作区间')
                value_11 = None
            
            
            # 原始版本：取第一个峰值区间，判定征服者起吊、入水
            # indices = [i for i, x in enumerate(data1) if x[1] == peak_list[0]]
            # if indices:
            #     idx = indices[0]
            #     # 向前和向后扩展以获取完整的通电流段（两边的数值均大于0）
            ...
            indices = overlap_events.index[overlap_events['Ajia-5_v_new'] == value_11].tolist()
            if not indices: 
                ...
                print(peak_list[0])
                return None
            df.loc[indices[0], 'status'] = '征服者起吊'

            value_rope = find_first_stable_after_peak(data1, chosen_peak)
            indices = overlap_events.index[overlap_events['Ajia-5_v_new'] == value_rope].tolist()
            if not indices:
                print('缆绳解除找不到索引',value_rope)
                print(segment)
                print(data1)
                print(peak_list[0])
                return None

            df.loc[indices[0], 'status'] = '缆绳解除'
            rope_time = df.loc[indices[0], 'csvTime']
            previous_indices = [idx - 1 for idx in indices if idx > 0]
            df.loc[previous_indices[0], 'status'] = '征服者入水'

            # 3.3 找到征服者入水后的最大峰值，标记为A架摆回
            # max_peak = max(peak_list[1:])
            # a_indices = overlap_events.index[overlap_events['Ajia-5_v_new'] == max_peak].tolist()
            # df.loc[a_indices[0], 'status'] = 'A架摆回'
            # 3.3 找到chosen_peak之后的最大peak，并标记为'A架摆回'
            try:
                chosen_index = peak_list.index(chosen_peak)
                if chosen_index < len(peak_list) - 1:
                    max_peak_after = max(peak_list[chosen_index+1:])
                else:
                    max_peak_after = None
            except ValueError:
                max_peak_after = None

            if max_peak_after is not None:
                a_indices = overlap_events.index[overlap_events['Ajia-5_v_new'] == max_peak_after].tolist()
                if a_indices:
                    df.loc[a_indices[0], 'status'] = 'A架摆回'
                else:
                    print(f"找不到A架摆回对应的索引，max_peak_after: {max_peak_after}")
            else:
                print(f"没有找到chosen_peak{chosen_peak}之后的峰值")

            # 3.4 将'缆绳挂妥'时刻传入device函数判定折臂吊车相关动作
            if rope_time:
                device(rope_time)
            # 3.5 判定完成，跳出循环
            success = True
        ...
        if success:
            print(f"下放阶段 {segment} processed successfully.")
        else:
            print(f"下放阶段 {segment} failed to process.")

    def huishou(segment):
        # if segment[0] == '2024-07-18 15:08:39': #2024/7/18 15:08:39
        #     print('debug')
        # Convert A架 segment start and end to datetime
        segment_start = pd.to_datetime(segment[0])
        segment_end = pd.to_datetime(segment[1])
        ...
        for crane_start, crane_end in crane_segments:
            ...
            if work_events.shape[0] < 6: # 该区间内的‘由待机进入工作’与‘由工作进入待机’事件数小于6
                print('折臂吊车区间内的‘由待机进入工作’与‘由工作进入待机’事件数小于6')
                continue
            ...
            # 调用extract_events函数
            events_list, peak_list = extract_events(df, (overlap_start_str, overlap_end_str), '回收')
            ...
            # 3.2 提取重叠区间内的A架数据，判定峰值、挂妥及落座动作
            data1 = overlap_events[['Ajia-3_v_new', 'Ajia-5_v_new']].values.tolist()
            # len_peaks, peak_list = find_peaks(data1, '回收', 0)
            # if not peak_list:
            #     continue

            # 在所有峰值中找最大峰值，并标记该行为'征服者出水'，但是征服者出水不能为第一个峰值，因为第一个峰值为A架摆出
            # if len(peak_list) < 2:
            #     continue
            # Exclude the first peak (A架摆出) and get the maximum of the rest
            max_peak = max(peak_list[1:])
            outwater_indices = overlap_events.index[overlap_events['Ajia-5_v_new'] == max_peak].tolist()
            if not outwater_indices:
                print('找不到征服者出水的索引',max_peak)
                print(segment)
                print(data1)
                continue
            idx_outwater = outwater_indices[0]
            df.loc[idx_outwater, 'status'] = '征服者出水'

            # 其前一行标记为'缆绳挂妥'
            if idx_outwater - 1 in overlap_events.index:
                df.loc[idx_outwater - 1, 'status'] = '缆绳挂妥'
                rope_time = df.loc[idx_outwater - 1, 'csvTime']
            else:
                rope_time = None

            # 利用find_first_stable_after_peak函数确定峰值后的稳定值，标记为'征服者落座'
            stable_val = find_first_stable_after_peak(data1, max_peak)
            if stable_val is not None:
                stable_indices = overlap_events.index[overlap_events['Ajia-5_v_new'] == stable_val].tolist()
                if stable_indices:
                    df.loc[stable_indices[0], 'status'] = '征服者落座'
            else:
                print('找不到征服者落座的索引',data1,max_peak)
                print(segment)
                continue

            # 3.3 在'缆绳挂妥'之前的区间内找第一个峰值，并标记为'A架摆出'
            # 使用已有的 peak_list 列表，查找在征服者出水（idx_outwater）之前的峰值
            candidate_peaks = []
            for peak in peak_list:
                # 获取该峰值在 overlap_events 中首次出现的索引
                peak_idxs = overlap_events.index[overlap_events['Ajia-5_v_new'] == peak].tolist()
                if peak_idxs:
                    peak_idx = peak_idxs[0]
                    if peak_idx < idx_outwater:
                        candidate_peaks.append((peak_idx, peak))
            if candidate_peaks:
                # 按照索引排序，取最靠近征服者出水（即最后一个满足条件的峰值） 如果征服者出水前有多个峰值，到底取哪一个？（去第一个、最后一个、最大的）
                candidate_peaks.sort(key=lambda x: x[0])
                target_idx, _ = candidate_peaks[-1]
                df.loc[target_idx, 'status'] = 'A架摆出'
        ...
        if success:
            print(f"回收阶段 {segment} processed successfully.")
        else:
            print(f"回收阶段 {segment} failed to process.")

    # 对每个有效开关机时间段进行分类处理（下放、回收） 前提：每个开关机时间段是有效的，即有多个峰值且折臂吊车开关机时间段和DP开启时间段有重叠
    for day, segments in segments_by_date.items():
        if len(segments) == 1:
            start_time_obj = pd.to_datetime(segments[0][0])
            if start_time_obj.hour < 12:
                if check_Ajia(segments[0][0], segments[0][1], '布放'):
                    xiafang(segments[0])
            else:
                if check_Ajia(segments[0][0], segments[0][1], '回收'):
                    huishou(segments[0])
        else:
            for idx, segment in enumerate(segments):
                if check_Ajia(segment[0],segment[1],'布放' if idx % 2 == 0 else '回收'):
                    if idx % 2 == 0:
                        xiafang(segment)
                    else:
                        huishou(segment)
